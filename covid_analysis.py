    ...
    def chart(self):
        ...
        if to_view in get_states():
            ...
            for rows in covid4.itertuples():
                my_list = [
                    rows.date.strftime("%m/%d/%y"),
                    str(rows.totalTestResultsIncrease),
                    str(rows.positiveIncrease),
                    str(rows.deathIncrease),
                    str(rows.hospitalizedIncrease),
                    str(
                        round(
                            (rows.positiveIncrease / rows.totalTestResultsIncrease)
                            * 100,
                            2,
                        )
                    ),
...
def plot_data(df, df3, to_view):
    if to_view in get_states():
        ...
        chart1 = df.plot(
            x="date",
            y=["change_in_cases", "change_in_deaths"],
            kind="bar",
            title="{} - in the last {} days".format(to_view, cut_off_days),
            use_index=True,
            logy=True,
